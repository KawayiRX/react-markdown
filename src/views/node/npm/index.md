...
* version 一般的格式是 x.x.x。是项目的版本
    ...
    * 第三部分为修订版本号，<Text type="danger">变化了表示有bug修复，并且可以向后兼容。</Text>
...
* 指定项目入口文件，假如模块名为'foo',当使用者使用require('foo'), 则main字段指定的模块导出对象会被返回。
...
```
* 这样当安装myappnpm会从index.js创建一个到 /usr/local/bin/myapp 的符号链接 (这使你可以直接在命令行执行 myapp)。
